	...
	for($i = 0; $i <count($categories); $i++){
		...
		for($j = 0; $j < count($subcategories) ; $j++){
			...
			for($k = 0; $k < count($keywords) ; $k+=4){
				...
				$categories_check_box .= "\r\n<tr><td><input type='checkbox' class='keywords_checkbox' id='K{$keyword_id}' name='K{$keyword_id}'><label for='K{$keyword_id}'>{$keyword_name}</label><input  style='display:none' type='text' id='MK{$keyword_id}' name='MK{$keyword_id}'></td>";
